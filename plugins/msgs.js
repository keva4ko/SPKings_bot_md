...
bot(
	...
	async (message, match) => {
		...
		for (const participant in participants) {
			msg += `*Nummer:* +${jidToNum(participant)}\n*Name:* ${
				participants[participant].name || ''
			}\n*Nachrichtenzahl:* ${participants[participant].total}\n`
...
bot(
	{
		pattern: 'inactive ?(.*)',
		fromMe: true,
		desc: 'show/kick who message count not met (NICHT BENUTZEN!!!)',
		type: 'group',
		onlyGroup: true,
	},
	async (message, match) => {
